...
export class RelendActionCore {
  ...
  static async initialize(
    ...
    const obligationAddress = customObligationAddress ?? (await PublicKey.createWithSeed(publicKey, seed, programId))
    ...
    const userTokenAccountAddress = await getAssociatedTokenAddress(new PublicKey(reserve.mintAddress), publicKey, true)
  ...
  static async buildForgiveTxns(
    ...
    const axn = await RelendActionCore.initialize(
      pool,
      reserve,
      "deposit",
      new BN(amount),
    ...
    this.lendingIxs.push(
      withdrawObligationCollateralInstruction(
        this.amount.eq(new BN(U64_MAX))
          ? this.amount
          : new BN(new BigNumber(this.amount.toString()).dividedBy(cTokenExchangeRate).toString()),
        new PublicKey(this.reserve.cTokenLiquidityAddress),
        this.userCollateralAccountAddress,
        new PublicKey(this.reserve.address),
        this.obligationAddress, // obligation
  ...
  private async addAtaIxs(action: ActionType) {
    ...
    if (action === "withdraw" || action === "mint" || action === "deposit") {
      ...
      if (!userCollateralAccountInfo) {
        ...
        if (this.positions === POSITION_LIMIT && this.reserve.mintAddress === NATIVE_MINT.toBase58()) {
