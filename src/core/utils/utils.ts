...
export const parseRateLimiter = (rateLimiter: RateLimiter, currentSlot: number) => {
  const convertedRateLimiter = {
    config: {
      windowDuration: new BigNumber(rateLimiter.config.windowDuration.toString()),
    ...
    previousQuantity: new BigNumber(rateLimiter.previousQuantity.toString()).shiftedBy(-18),
    currentQuantity: new BigNumber(rateLimiter.currentQuantity.toString()).shiftedBy(-18),
  }
  return {
...
export const remainingOutflow = (
  ...
  const curSlotStart = curSlot.dividedBy(windowDuration).integerValue(BigNumber.ROUND_FLOOR).times(windowDuration)

  const prevWeight = windowDuration.minus(curSlot.minus(curSlotStart).plus(new BigNumber(1))).dividedBy(windowDuration)
...
export const formatAddress = (address: string, length?: number) => {
  return `${address.slice(0, length ?? ADDRESS_PREFIX_SUFFIX_LENGTH)}...${address.slice(
...
export async function getBatchMultipleAccountsInfo(addresses: Array<string | PublicKey>, connection: Connection) {
  ...
  return (
    await Promise.all(res.map((accountGroup) => connection.getMultipleAccountsInfo(accountGroup, "processed")))
  ).flatMap((x) => x)
}

export async function createObligationAddress(publicKey: string, marketAddress: string, programId: string) {
  return (
    await PublicKey.createWithSeed(new PublicKey(publicKey), marketAddress.slice(0, 32), new PublicKey(programId))
  ).toBase58()
}
