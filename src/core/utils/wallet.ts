import { getAssociatedTokenAddress, NATIVE_MINT, unpackAccount } from "@solana/spl-token"
...
export function formatWalletAssets(
  ...
  const assets = userAssociatedTokenAccounts
    .map((parsedAccount) => {
      ...
      return {
        decimals,
        symbol: tokenMetadata?.symbol === "SOL" ? "wSOL" : tokenMetadata?.symbol,
        address: parsedAccount.address.toBase58(),
        amount: new BigNumber(parsedAccount.amount.toString()).shiftedBy(-decimals),
...
export async function fetchWalletAssets(
  ...
  const userTokenAssociatedAddresses = await Promise.all(
    uniqueAssets.map(async (asset) => {
      const userTokenAccount = await getAssociatedTokenAddress(new PublicKey(asset), new PublicKey(publicKey), true)
      return userTokenAccount
    })
  )

  const userAssociatedTokenAccounts = await getBatchMultipleAccountsInfo(userTokenAssociatedAddresses, connection)

  const nativeSolBalance = await connection.getBalance(new PublicKey(publicKey))

  const wSolAddress = (await getAssociatedTokenAddress(NATIVE_MINT, new PublicKey(publicKey), true)).toBase58()

  return {
    userAssociatedTokenAccounts: userAssociatedTokenAccounts.map((account, index) =>
      account ? unpackAccount(userTokenAssociatedAddresses[index], account) : null
    ),
    nativeSolBalance: new BigNumber(nativeSolBalance).dividedBy(LAMPORTS_PER_SOL),
    wSolAddress,
  }
}
