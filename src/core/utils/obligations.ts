...
export function formatObligation(obligation: { pubkey: PublicKey; info: Obligation }, pool: PoolType) {
  ...
  const deposits = obligation.info.deposits
    ...
    .map((d) => {
      ...
      if (!reserve) throw Error("Deposit in obligation does not exist in the pool")
  ...
  const borrows = obligation.info.borrows
    ...
    .map((b) => {
      ...
      const reserve = pool.reserves.find((r) => r.address === reserveAddress)
      if (!reserve) throw Error("Borrow in obligation does not exist in the pool")

      const amount = new BigNumber(b.borrowedAmountWads.toString())
        .shiftedBy(-18 - reserve.decimals)
        .times(reserve.cumulativeBorrowRate)
        .dividedBy(new BigNumber(b.cumulativeBorrowRateWads.toString()).shiftedBy(-18))
      const amountUsd = amount.times(reserve.price)
      ...
      maxPriceUserTotalWeightedBorrow = maxPriceUserTotalWeightedBorrow.plus(
        amount.times(maxPrice).times(reserve.borrowWeight ? reserve.borrowWeight : U64_MAX)
      )
  ...
  const totalSupplyValue = deposits.reduce((acc, d) => acc.plus(d.amountUsd), new BigNumber(0))
  const totalBorrowValue = borrows.reduce((acc, b) => acc.plus(b.amountUsd), new BigNumber(0))
  const weightedTotalBorrowValue = borrows.reduce((acc, b) => acc.plus(b.weightedAmountUsd), new BigNumber(0))

  const borrowLimit = deposits.reduce((acc, d) => d.amountUsd.times(d.loanToValueRatio).plus(acc), BigNumber(0))
  ...
  const isBorrowLimitReached = borrowUtilization.isGreaterThanOrEqualTo(new BigNumber("1"))
  ...
  const positions =
    ...
    obligation.info.borrows.filter((b) => !b.borrowedAmountWads.isZero()).length
...
export async function fetchObligationByAddress(obligationAddress: string, connection: Connection, debug?: boolean) {
  if (debug) console.log("fetchObligationByAddress")

  const rawObligationData = await connection.getAccountInfo(new PublicKey(obligationAddress))

  if (!rawObligationData) {
    return null
  }

  const parsedObligation = parseObligation(new PublicKey(obligationAddress), rawObligationData!)
...
export async function fetchObligationsByAddress(
  ...
  if (debug) console.log("fetchObligationsByAddress", obligationAddresses.length)
  ...
  const parsedObligations = rawObligations
    .map((obligation, index) =>
      obligation ? parseObligation(new PublicKey(obligationAddresses[index]), obligation) : null
    )
