...
const calculateUtilizationRatio = (reserve: Reserve) => {
  const borrowedAmount = new BigNumber(reserve.liquidity.borrowedAmountWads.toString()).shiftedBy(-18)
...
const calculateBorrowAPR = (reserve: Reserve) => {
  ...
  const maxUtilizationRate = new BigNumber(reserve.config.maxUtilizationRate / 100)
  let borrowAPR
  if (currentUtilization.isLessThanOrEqualTo(optimalUtilization)) {
    ...
    borrowAPR = normalizedFactor.times(optimalBorrowRate.minus(minBorrowRate)).plus(minBorrowRate)
  } else if (currentUtilization.isLessThanOrEqualTo(maxUtilizationRate)) {
    const weight = currentUtilization.minus(optimalUtilization).dividedBy(maxUtilizationRate.minus(optimalUtilization))
    ...
    const weight = currentUtilization.minus(maxUtilizationRate).dividedBy(new BigNumber(1).minus(maxUtilizationRate))

    const maxBorrowRate = new BigNumber(reserve.config.maxBorrowRate / 100)
    const superMaxBorrowRate = new BigNumber(reserve.config.superMaxBorrowRate / 100)

    borrowAPR = weight.times(superMaxBorrowRate.minus(maxBorrowRate)).plus(maxBorrowRate)
  }
