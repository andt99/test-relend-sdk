...
export class InMemoryCache implements RPCCacheInterface {
  ...
  async set(key: string, valuePromise: Promise<any>) {
    ...
    if (this.maxCacheSize !== null && Object.keys(this.data).length === this.maxCacheSize) {
...
export class CachedConnection implements RelendRPCConnection {
  connection: RelendRPCConnection
  cache: RPCCacheInterface
  public rpcEndpoint: string

  constructor(connection: RelendRPCConnection, cache: RPCCacheInterface) {
  ...
  async getLatestBlockhash(
    ...
    return (
      (await this.cache.get(key)) || (await this.cache.set(key, this.connection.getLatestBlockhash(commitmentOrConfig)))
    )
  }
  ...
  async getMultipleAccountsInfo(
    ...
    const key = `getMultipleAccountsInfo_${publicKeys.map((pk) => pk.toBase58).join("_")}_${JSON.stringify(
      commitmentOrConfig
    )}`
    return (
      (await this.cache.get(key)) ||
  ...
  async getRecentBlockhash(commitment?: Commitment): Promise<{
    ...
    return (await this.cache.get(key)) || (await this.cache.set(key, this.connection.getRecentBlockhash(commitment)))
  }

  async getSlot(commitmentOrConfig?: Commitment | GetSlotConfig): Promise<number> {
    const key = `getSlot_${commitmentOrConfig}`
    return (await this.cache.get(key)) || (await this.cache.set(key, this.connection.getSlot(commitmentOrConfig)))
  }
  ...
  async getTransaction(
    ...
    return (
      (await this.cache.get(key)) || (await this.cache.set(key, this.connection.getTransaction(signature, rawConfig)))
    )
  }

  // This does not make sense to cache, so we don't
  async sendTransaction(transaction: VersionedTransaction, options?: SendOptions): Promise<TransactionSignature> {
