...
const main = async () => {
  ...
  const lendingMarketKey = new PublicKey("HB1cecsgnFPBfKxEDfarVtKXEARWuViJKCqztWiFB3Uk")

  // main pool
  const market = await RelendMarket.initialize(connection, "beta", lendingMarketKey.toString())

  const solReserve = market.reserves.find((res) => res.config.liquidityToken.symbol == "SOL")
  ...
  // turbo sol pool
  const marketTurbo = await RelendMarket.initialize(connection, "beta", "Az4MpWtMcpENQZwbEbTnrgyd2qk3wsMwQXimadUiHSQp")
  const solReserveTurbo = marketTurbo.reserves.find((res) => res.config.liquidityToken.symbol == "SOL") ?? null
  ...
  const solATA = await getATA(new PublicKey(solReserve.config.liquidityToken.mint), payer.publicKey)
