...
export class RelendObligation {
  ...
  private calculatePositions(obligation: Obligation, reserves: Array<RelendReserve>) {
    ...
    const deposits = obligation.deposits.map((deposit) => {
      ...
      const supplyAmount = new BN(
        Math.floor(
          new BigNumber(deposit.depositedAmount.toString()).multipliedBy(reserve!.stats!.cTokenExchangeRate).toNumber()
        )
      )
