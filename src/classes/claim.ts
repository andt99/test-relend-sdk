...
export class RelendClaim {
  ...
  async getExerciseIxs(amount: number) {
    ...
    const optionMarket = await getOptionByKey(psyOptionsProgram, new PublicKey(this.metadata.optionMarketKey))
    ...
    const claimantTokenAccountInfo = await this.provider.connection.getAccountInfo(claimantTokenAccountAddress)
    ...
    const quoteAssetSrc = await getAssociatedTokenAddress(optionMarket.quoteAssetMint, this.provider.wallet.publicKey)
  ...
  async claim() {
    ...
    return await this.provider.sendAndConfirm(new Transaction().add(...setupIxs, ...claimIxs))
  }
}
