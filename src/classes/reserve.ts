...
export class RelendReserve {
  ...
  private calculateSupplyAPY = (reserve: ParsedReserve) => {
    ...
    const apy = new BigNumber(1).plus(new BigNumber(apr).dividedBy(SLOTS_PER_YEAR)).toNumber() ** SLOTS_PER_YEAR - 1
    return apy
  }
  ...
  private calculateUtilizationRatio(reserve: ParsedReserve) {
    const totalBorrowsWads = new BigNumber(reserve.liquidity.borrowedAmountWads.toString()).div(WAD)
  ...
  private calculateBorrowAPR(reserve: ParsedReserve) {
    ...
    if (optimalUtilization === 1.0 || currentUtilization < optimalUtilization) {
      ...
      borrowAPR = normalizedFactor * (maxBorrowRate - optimalBorrowRate) + optimalBorrowRate
    }
  ...
  async load(rewardsData?: RewardsDataType) {
    ...
    if (!this.buffer) {
      this.buffer = await this.connection.getAccountInfo(new PublicKey(this.config.address), "processed")
    }
    ...
    const parsedData = parseReserve(new PublicKey(this.config.address), this.buffer)?.info
    if (!parsedData) {
  ...
  calculateRewardAPY(rewardRate: string, poolSize: string, rewardPrice: number, tokenPrice: number, decimals: number) {
    const poolValueUSD = new BigNumber(poolSize)
      .times(tokenPrice)
      .dividedBy("1".concat(Array(decimals + 1).join("0")))
      .dividedBy(WAD)

    return new BigNumber(rewardRate).multipliedBy(rewardPrice).dividedBy(poolValueUSD).dividedBy(WANG)
  }

  totalSupplyAPY() {
    const { stats } = this
    if (!this.rewardsData || !stats) {
      throw Error("RelendMarket must call loadRewards.")
    }
    ...
    const totalAPY = new BigNumber(stats.supplyInterestAPY)
      .plus(rewards.reduce((acc, reward) => acc.plus(reward.apy), new BigNumber(0)))
      .toNumber()
  ...
  totalBorrowAPY() {
    ...
    const totalAPY = new BigNumber(stats.borrowInterestAPY)
      .minus(rewards.reduce((acc, reward) => acc.plus(reward.apy), new BigNumber(0)))
      .toNumber()
  ...
  private formatReserveData(parsedData: NonNullable<ReturnType<typeof parseReserve>>["info"]): ReserveDataType {
    ...
    return {
      ...
      borrowFeePercentage: new BigNumber(parsedData.config.fees.borrowFeeWad.toString()).dividedBy(WAD).toNumber(),
      hostFeePercentage: parsedData.config.fees.hostFeePercentage / 100,
      flashLoanFeePercentage: new BigNumber(parsedData.config.fees.flashLoanFeeWad.toString())
        .dividedBy(WAD)
        .toNumber(),
      ...
      assetPriceUSD: new BigNumber(parsedData.liquidity.marketPrice.toString()).div(WAD).toNumber(),
