...
export const parseObligation = (pubkey: PublicKey, info: AccountInfo<Buffer>, encoding?: string) => {
  ...
  const deposits = BufferLayout.seq(ObligationCollateralLayout, depositsLen).decode(
    depositsBuffer
  ...
  const borrows = BufferLayout.seq(ObligationLiquidityLayout, borrowsLen).decode(borrowsBuffer) as ObligationLiquidity[]
