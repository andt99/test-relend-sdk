...
import { parseReserve, RelendAction, RelendMarket, RelendWallet } from "../src";
...
describe("calculate", function () {
  ...
  it("reads wallet", async function () {
    const connection = new Connection(network, {
      ...
      const solendWallet = await RelendWallet.initialize(
        wallet,
        connection,
      );
  ...
  it("performs a deposit", async function () {
    ...
    const solendAction = await RelendAction.buildDepositTxns(
      connection,
      depositAmount,
      "SOL",
      account.publicKey,
